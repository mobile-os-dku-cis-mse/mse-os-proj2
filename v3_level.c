...
struct TwoLevel{
	int valid; //0:invlid 1:valid
	struct PageTableEntry pte[0x400];
};
struct PageTable{
	pid_t pid;
	struct TwoLevel level[0x400];
	struct PCB pcb;
};
...
*/
	...
	//va --> pa addr trans
	for(int i=0; i<10; i++){
		pageN = msg.acc_rq[i]>>22;
		pageN2 = (msg.acc_rq[i]>>12)&0x3ff;
		offset = msg.acc_rq[i]&0xfff;

		//first level page table check
		if(pt[curr].level[pageN].valid == 0){
			//invalid
			...
			pt[curr].level[pageN].pte[pageN2].pfn = frameN;
			pt[curr].level[pageN].pte[pageN2].valid = 1;
			pt[curr].level[pageN].valid = 1;

			//write
			pa = frameN*0x1000+offset;

		}
		else{
			//second level page table check
			if(pt[curr].level[pageN].pte[pageN2].valid == 0){
				//invalid
				//page fault!
				...
				//valid
				//calculate PA
				frameN = pt[curr].level[pageN].pte[pageN2].pfn;
